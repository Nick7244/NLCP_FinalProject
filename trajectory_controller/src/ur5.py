...
class robot_config:
    ...
    def __init__(self, config_folder):
        ...
        self._Cq = None  # placeholder for joint space inertia matrix function
        ...
        self._M.append(np.diag([4.0, 4.0, 4.0,
                                0.0044, 0.0044, 0.0072]))  # link0



        #self._M.append(np.diag([2.5, 2.5, 2.5,
        #                        0.04, 0.04, 0.04]))  # link1
        self._M.append(np.diag([3.7000, 3.7000, 3.7000,
                                0.0103, 0.0103, 0.0067]))  # link1



        #self._M.append(np.diag([5.7, 5.7, 5.7,
        #                        0.06, 0.06, 0.04]))  # link2
        self._M.append(np.diag([8.3930, 8.3930, 8.3930,
                                0.2269, 0.2269, 0.0151]))  # link2



        #self._M.append(np.diag([3.9, 3.9, 3.9,
        #                        0.055, 0.055, 0.04]))  # link3
        self._M.append(np.diag([2.2750, 2.2750, 2.2750,
                                0.0312, 0.0312, 0.0041]))  # link3



        # self._M.append(np.copy(self._M[1]))  # link4
        self._M.append(np.diag([1.2190, 1.2190, 1.2190,
                                0.0026, 0.0026, 0.0022]))  # link4



        # self._M.append(np.copy(self._M[1]))  # link5
        self._M.append(np.diag([1.2190, 1.2190, 1.2190,
                                0.0026, 0.0026, 0.0022]))  # link5



        #self._M.append(np.diag([0.7, 0.7, 0.7,
        #                        0.01, 0.01, 0.01]))  # link6
        self._M.append(np.diag([0.1879, 0.1879, 0.1879,
                                8.4696e-05, 8.4696e-05, 1.3212e-04]))  # link6
        ...
        self.T12 = sp.Matrix([
            ...
            [0, 1, 0, -0.1197],
            [-sp.sin(-self.q[2]), 0, sp.cos(-self.q[2]), 0.425],
            [0, 0, 0, 1]])
        ...
        # transform matrix from joint 2 to joint 3
        self.T23 = sp.Matrix([
            [-sp.sin(-self.q[3]), 0, sp.cos(-self.q[3]), 0],
            [0, 1, 0, 0],
            [-sp.cos(-self.q[3]), 0, -sp.sin(-self.q[3]), 0.39225],
            [0, 0, 0, 1]])
        ...
        # transform matrix from joint 3 to joint 4
        self.T34 = sp.Matrix([
            [sp.cos(-self.q[4]), 0, sp.sin(-self.q[4]), 0],
            [0, 1, 0, 0.093],
            [-sp.sin(-self.q[4]), 0, sp.cos(-self.q[4]), 0],
            [0, 0, 0, 1]])
        ...
        if os.path.isfile('%s/%s.J' % (self.config_folder, name)):
            ...
            if end_point != 'EE':
                ...
                for ii in range(end_point):
                    # print(list(self.J_orientation[ii]))
    ...
    def _calc_Cq(self, lambdify=True):
        """ Uses Sympy to generate the coriolis matrix in
        joint space for the ur5
        lambdify boolean: if True returns a function to calculate
                          the Jacobian. If False returns the Sympy
                          matrix
